...
#ifdef DHD_DEBUG
#ifndef SRCBASE
#define SRCBASE        "drivers/amlogic/wifi/broadcm_4329"
#endif
#define DHD_COMPILED "\nCompiled in " SRCBASE
#else
#define DHD_COMPILED
...
#define DHD_REQUEST_FIRMWARE 1
#ifdef DHD_REQUEST_FIRMWARE

extern struct device sdmmc_dev;

static unsigned int fw_fpos=0;
void *
dhd_os_open_image(char *filename)
{
	const struct firmware *fw;
	struct device dev_dummy;
	int ret;

	memset((void*)&dev_dummy, 0, sizeof(struct device));
	ret = request_firmware(&fw, filename, &dev_dummy);
	if (ret) {
		printk(KERN_ERR "dhd cdc: Failed to load firmware \"%s\"\n",
			filename);
		return NULL;
	}
...
int
dhd_os_get_image_block(char *buf, int len, void *fw)
{
	struct firmware* firmw = (struct firmware*)fw;
	unsigned int rdlen=0;

	if(fw_fpos>= firmw->size)
		rdlen = 0;
	else if(fw_fpos+len > firmw->size)
		rdlen = firmw->size - fw_fpos;
	else
		rdlen = len;
	
	memcpy(buf, firmw->data+fw_fpos, rdlen);
	fw_fpos += rdlen;
	return rdlen;
}

void
dhd_os_close_image(void *fw)
{
	if (fw)
