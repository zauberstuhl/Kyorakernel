...
static inline int _gpio_setup_bank_bit(cmd_t  *op)
{
	...
	{
		case 'a': //bank a
		op->bank=GPIOA_bank_bit(0);
		if(op->bit < 15) //bit0..14
		{
			op->bit=GPIOA_bit_bit0_14(op->bit);
		}
		else
		{
			if(op->bit >22 && op->bit < 27) //bit23..26
			{
				op->bit=GPIOA_bit_bit23_26(op->bit);
			}
			else
			{
				return -1;
		...
		break;
		case 'b': //bank b
		op->bank=GPIOB_bank_bit0_7(0);
		if(op->bit < 8)//bit0..7
		{
			op->bit =GPIOB_bit_bit0_7(op->bit);
		}
		else
		{
			return -1;
		...
		break;
		case 'c': //bank c
		op->bank=GPIOC_bank_bit0_26(0);
		if(op->bit < 27)//bit0..26
		{
			op->bit =GPIOC_bit_bit0_26(op->bit);
		}
		else
		{
			return -1;
		...
		break;
		case 'e': //bank e
		op->bank=GPIOE_bank_bit0_15(0);//bit 0..15 16..21 share one bank
		if(op->bit > 21)//bit2..24 ,bit no change .
		{
			return -1;
		}
		break;	
